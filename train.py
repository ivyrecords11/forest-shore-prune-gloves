...
class CerebellarNet_old(nn.Module):
    ...
    def __init__(self, cfg: SimulationConfig):
        ...
        self.pkj = neuron.LIFnode(tau=cfg.t_pkj, threshold = cfg.th_pkj, surrogate_function=surrogate.ATan(), detach_reset=True)
        ...
        self.pkj2mo = nn.Linear(cfg.n_pkj * 4 * 4, 4, bias=False)
        self.pkj2critic = nn.Linear(cfg.n_pkj * 4 * 4, 4, bias=True)
    ...
    def forward(self, mf: torch.Tensor, cf: torch.Tensor):
        ...
        pkj = self.pkj(F.flatten(grc2pkj - mli2pkj - cf2pkj))             # (N, n_pkj, 4, 4)

        motor = self.motor(self.pkj2mo(pkj.reshape(pkj.shape[0], -1))) # (N, 4)
        return motor.v
    #, {"goc": goc, "grc": grc, "mli": mli, "pkj": pkj}
...
from spikingjelly_wrapper.spikingjelly.activation_based import neuron, surrogate
from typing import Tuple

# If you already have this class elsewhere, keep that import and remove this placeholder.
class NonSpikingLIFNode(neuron.LIFNode):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
    def forward(self, x):
...
class CerebellarNet(nn.Module):
    """
    Pipeline (spatial dims in comments):
      goc = Conv3x3(mf)                                      -> (N, n_goc, 8, 8)
      goc_pool = avg_pool2d(goc, 2)                          -> (N, n_goc, 4, 4)   # NEW

      grc = Conv5x5(mf) + Conv3x3(goc)                       -> (N, n_grc, 6, 6)
      mli = Conv3x3(grc)                                     -> (N, n_mli, 4, 4)

      pkj_pre = Conv3x3(grc)->n_pkj + Conv1x1(mli)->n_pkj + FC(cf)->(n_pkj,4,4) -> (N, n_pkj, 4, 4)
      pkj_act = LIF(pkj_pre)                                 -> (N, n_pkj, 4, 4)
      pkj_pool = avg_pool2d(pkj_act, 2)                      -> (N, n_pkj, 2, 2)   # NEW

      motor = FC( flatten(pkj_pool) ) -> 4                   -> (N, 4)
      critic = FC( flatten(pkj_pool) ) -> 4                  -> (N, 4)  # if used
    """
    def __init__(self, cfg):
        super().__init__()
        # Spiking / non-spiking nodes
        self.grc  = neuron.LIFnode(tau=cfg.t_grc,  surrogate_function=surrogate.ATan(), detach_reset=True)
        self.goc  = neuron.LIFnode(tau=cfg.t_goc,  surrogate_function=surrogate.ATan(), detach_reset=True)
        self.pkj  = neuron.LIFnode(tau=cfg.t_pkj,  threshold=cfg.th_pkj, surrogate_function=surrogate.ATan(), detach_reset=True)
        self.mli  = neuron.LIFnode(tau=cfg.t_mli,  surrogate_function=surrogate.ATan(), detach_reset=True)
        ...
        # CF -> PKJ (makes a (n_pkj,4,4) map that will be pooled later with others)
        self.cf2pkj = nn.Linear(cfg.n_cf, cfg.n_pkj * 4 * 4, bias=False)

        # NOTE: pkj feature map is pooled to (2,2), so linears expect n_pkj*2*2
        self.pkj2mo     = nn.Linear(cfg.n_pkj * 2 * 2, 4, bias=False)
        self.pkj2critic = nn.Linear(cfg.n_pkj * 2 * 2, 4, bias=True)
    ...
    def forward(self, mf: torch.Tensor, cf: torch.Tensor) -> torch.Tensor:
        ...
        motor    = self.motor(self.pkj2mo(flat_pkj))      # (N, 4)
