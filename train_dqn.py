...
def dqn_optimize(model, target, buffer, optimizer, batch_size, gamma, max_grad_norm, device):
    ...
    # --- Q(s): forward T steps, use actor membrane potential as Q
    from spikingjelly.activation_based import functional
    functional.reset_net(model.actor)
    for _ in range(model.T):
        _ = model.actor(s)
    q_s = model.actor[-1].v  # (B, A), no tanh/no clip
    ...
    with torch.no_grad():
        ...
        for _ in range(target.T):
            _ = target.actor(ns)
        q_ns_target = target.actor[-1].v  # (B, A)
...
def train_dqn(env, model, *,
    ...
    n_actions = model.actor[-2].out_features
