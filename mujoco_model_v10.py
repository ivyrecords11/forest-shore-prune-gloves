...
10. mujoco motor damping is adjustable now
11. spike rate all same within same block
12. info에 motor_acc 반환
13. negative reward 없앴음
"""
...
class Environment(Env):
    ...
    def return_xml(self):
        ...
        <worldbody>
            ...
            <joint name="lift_z" type="slide" axis="0 0 1"
                    range="-0.05 0.05" damping="{self.cfg.motor_damping}"/>

            <!-- 판 기울기: 두 각도 -->
            <joint name="hinge_x" type="hinge" axis="1 0 0"
                    damping="{self.cfg.motor_damping}"
    ...
    def sensor_inputs(self, flatten=True):
        ...
        # --- 거리 및 가우시안 감쇠 (완전 벡터화) ---
        dist2 = ((ball_x_t - self.gx-0.015) ** 2 + (ball_y_t - self.gy-0.015) ** 2) / 0.18
