...
from snn_model import CerebellarCNNAC2, SpikingNet       # <- CerebellarCNNAC2 정의가 있는 모듈로 교체
...
# =========================
@torch.no_grad()
def q_values_from_model(model, obs_batch, cf):
    # obs_batch: (B, 1, H, W)
    assert obs_batch.shape[1:] == (1,10,10), f"[!]obs_batch:{obs_batch}"
    functional.reset_net(model)
    for _ in range(model.T):
        a = model.forward(obs_batch, cf)
        #print(obs_batch, action)
    #assert a.shape == torch.Size([4]), f"q[0]: {a[0]}, q: {a}"
    #print("==========================================================")
    #print(action)
...
def dqn_optimize(model, target, buffer, optimizer, batch_size, gamma, max_grad_norm, device):
    """
    Multi-output DQN update for vector controllers (e.g., 4 outputs) with
    recurrent-style forward: model.forward(x, prev_q).
    Trains all A dims at once: y = r + gamma*(1-d)*q_next_target
    Loss = SmoothL1(q_s, y) elementwise over (B, A).
    """
    if len(buffer) < batch_size:
        return None
    model.train(True)

    # s: (B, 1, H, W), a: (B, A), r: (B,), ns: (B, 1, H, W), d: (B,)
    s, a, r, ns, d = buffer.sample(batch_size)
    s, ns = s.to(device), ns.to(device)
    r, d  = r.to(device), d.to(device)

    B = s.shape[0]
    A = a.shape[1]  # use action tensor to infer output dim

    from spikingjelly.activation_based import functional

    # --- Q(s): run T steps; feed previous output into model.forward each step
    functional.reset_net(model)
    prev_q = torch.zeros(B, A, device=device)
    for _ in range(model.T):
        q_s = model.forward(s, prev_q)
        prev_q = q_s.detach()  # detach to prevent gradient through time unless needed

    # --- Target Q(ns): elementwise TD target (no argmax)
    with torch.no_grad():
        functional.reset_net(target)
        prev_q_t = torch.zeros(B, A, device=device)
        for _ in range(target.T):
            q_ns_target = target.forward(ns, prev_q_t)
            prev_q_t = q_ns_target

        # y = r + gamma*(1-d)*q_ns_target, broadcast r,d to (B, A)
        y = r.unsqueeze(1) + gamma * (1.0 - d).unsqueeze(1) * q_ns_target
...
def select_action_epsilon_greedy(model, obs, cf, eps, n_actions, device):
    """
    ...
    with torch.no_grad():
        ...
        q_values = q_values_from_model(model, obs_t, cf).squeeze(0)        # (n_actions,)
        #assert q_values[0].shape == torch.Size([n_actions]), f"q_values.shape은 {q_values.shape}임"
        # obs_batch도 가능, 근데 단일임.
    # Convert to numpy for env
    #q_values = q_values.squeeze()
    action = q_values.clone().mean(axis=0)
    ...
    return action.cpu().numpy(), q_values  # shape: (n_actions,)
...
def train_dqn(env, model, *,
    ...
    for ep in range(episodes):
        ...
        while not done and steps < max_steps_per_ep:
            ...
            a, cf_next = select_action_epsilon_greedy(model, obs, cf, eps, n_actions, device)
        ...
        bx = last_info.get("ball_x", np.nan)
        by = last_info.get("ball_y", np.nan)
        ball_pos = (bx, by)
...
if __name__ == '__main__':
    ...
    if last_trial_line:
        ...
        if c.lower() == "y":
            ...
            model_pth = f"{DIR}/{trial_num}_train/model_params_{start_epoch}.pth"
