...
from snn_model import SpikingCNN# <- CerebellarCNNAC2 정의가 있는 모듈로 교체
...
def q_values_from_model(model, obs_batch):
    ...
    assert q[-1][-1].shape == torch.Size([4]), f"q[0]: {q[0]}, q: {q}"
    #print("==========================================================")
    #print(action)
    return q[-1][-1] # (B, n_actions)
...
def train_dqn(env, model, *,
    ...
    for ep in range(episodes):
        ...
        with open(log_csv, "a", newline="") as file_train_log:
            if new_file:
                file_train_log.write("epoch,datetime,duration,total_reward,ball_x,ball_y,ball_mass_g,terminated,truncated,total_reward_per_L1_dist\n")
            file_train_log.write(
                f"{epoch},{month:02}/{day:02} {hour:02}:{minute:02}:{second:02},{duration},{total_reward},{ball_pos[0]},{ball_pos[1]},{(ball_mass*1000) if not np.isnan(ball_mass) else ''},{terminated},{truncated},{total_reward/abs(ball_pos[0]+ball_pos[1])}\n"
            )
        gc.collect()
